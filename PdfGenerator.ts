...
export class PdfGenerator {
  ...
  public generateTasksPdf(tasks: Task[]): void {
    try {
  ...
  private addTasksToDocument(doc: any, tasks: Task[], startY: number): number {
    let y = startY;
    ...
    tasks.forEach((task, index) => {
      ...
      doc.text(`• Deadline: ${DateUtils.formatDateForDisplay(task.deadline)}`, 18, y);
      y += 5;
